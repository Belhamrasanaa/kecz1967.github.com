...
if ($id_entreprise) {
    ...
    if ($result->num_rows > 0) {
        ...
        $_SESSION['message'] = "Entreprise non trouvée.";
    }
    $stmt->close();
}
...
if ($cin) {
    ...
    if ($result->num_rows > 0) {
        ...
        $_SESSION['message'] = "Exploitant non trouvé.";
    }
    $stmt->close();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    header("Location: index2.php?id_entreprise=$id_entreprise&cin=$cin");
    exit();
}
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>Modifier l'entreprise et l'exploitant</title>
    <style>
   body {
        ...
        .form-container {
            ...
            width: 85%; /* Réduit la largeur du formulaire */
...
<body>
    ...
    <div class="form-container">
        <br>
        <h2>Modifier l'entreprise et l'exploitant</h2>
        <?php
        ...
        ?>
        <form action="form.php?id_entreprise=<?php echo htmlspecialchars($id_entreprise); ?>&cin=<?php echo htmlspecialchars($cin); ?>" method="POST">
            <!-- Formulaire pour l'entreprise -->
            <h2><pre>      Modifier l'entreprise:</pre></h2>
            <br>
...
<input list="types" name="type" id="type" value="<?php echo htmlspecialchars($entreprise['type'] ?? ''); ?>" required>

<!-- Liste des types disponibles -->
<datalist id="types">
    <option value="Industrielle">
    ...
    <option value="Autre">
            ...
            <h2><pre>      Modifier l'exploitant:<pre></h2>
            <br>
            ...
            <input type="text" name="adresse_exploitant" id="adresse_exploitant" value="<?php echo htmlspecialchars($exploitant['adresse_exploitant'] ?? ''); ?>" required>

            <label for="email_exploitant">Email:</label>
            <input type="email" name="email_exploitant" id="email_exploitant" value="<?php echo htmlspecialchars($exploitant['email_exploitant'] ?? ''); ?>" required>

            <label for="type_exploitant">Type de l'exploitant:</label>
<input list="types_exploitant" name="type_exploitant" id="type_exploitant" value="<?php echo htmlspecialchars($exploitant['type_exploitant'] ?? ''); ?>" required>

<!-- Liste des types d'exploitants disponibles -->
<datalist id="types_exploitant">
    <option value="Gérant">
